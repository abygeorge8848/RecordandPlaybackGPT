...
def reformat_paf_activity(event_queue, activity_name, activity_description):
    ...
    for event in event_queue:
        if event["event"] == "WaitForPageLoad":
            ...
            PAF_SCRIPT += '\t<alert type="accept"></alert>\n'
        elif event["event"] == "alert-cancel":
            PAF_SCRIPT += '\t<alert type="cancel"></alert>\n'
        elif event["event"] == "alert-getText":
            keyName = event["keyName"]
            PAF_SCRIPT += f'\t<alert type="getText" keyName="{keyName}"></alert>\n'
        elif event["event"] == "alert-input":
            value = event["value"]
            PAF_SCRIPT += f'\t<alert type="getText" value="{value}"></alert>\n'
        elif event["event"] == "alert-authenticate":
            user = event["user"]
            pwd = event["pwd"]
            PAF_SCRIPT += f'\t<alert type="getText" user="{user}" pwd="{pwd}"></alert>\n'
            ...
            VALIDATION_SCRIPT += f'\t<validate variable="{variable1}" condition="{conditions[condition]}" value="{variable2}" passMsg="{passMsg}" failMsg="{failMsg}"></validate>\n'
            VALIDATION_SCRIPT += f'</valGroup>\n'
